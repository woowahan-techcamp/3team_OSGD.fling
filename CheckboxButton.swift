...
open class CheckboxButton: UIControl {
    ...
    internal var containerFrame: CGRect {
        ...
        return CGRect(x: x + halfLineWidth, y: y + halfLineWidth,
                      width: sideLength - containerLineWidth, height: sideLength - containerLineWidth)
    }
    ...
    open override func endTracking(_ touch: UITouch?, with event: UIEvent?) {
        ...
        let validBounds = CGRect(x: bounds.origin.x - offset, y: bounds.origin.y - offset,
                                 width: bounds.width + (2 * offset), height: bounds.height + (2 * offset))
